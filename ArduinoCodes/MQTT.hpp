...
const char* MQTT_BROKER_ADRESS = "10.22.191.55";
const uint16_t MQTT_PORT = 1883;
const char* MQTT_CLIENT_NAME = "ESP8266Client_6";
...
void SuscribeMqtt()
{
	mqttClient.subscribe("arduino_6/sensor_deteccion"); // Suscribirse al tema para la deteccion del carro
  mqttClient.subscribe("arduino_6/led_state");  // Suscribirse al tema para el estado de los LEDs
}
...
void PublisMqtt(unsigned long data)
{
  ...
  mqttClient.publish("arduino_6/sensor_deteccion", (char*)payload.c_str());
...
void PublisMqttLedState(const String& ledColor, int timeOn)
{
    ...
    mqttClient.publish("arduino_6/led_state", payload.c_str());  // Publicar en el tema "arduino_1/led_state"
